...
export const OrderDetail: React.FC = () => {
  ...
  const canCancelOrder = Boolean(order && ['pending', 'confirmed'].includes(order.status));
  const showCancelButton = Boolean(user && canCancelOrder);
  ...
  const orderDetailContent = (
    <>
      <div className="mb-8">
        <Link to={user ? '/orders' : '/'} className="inline-flex items-center gap-2 text-[#C9A24D] hover:underline mb-4">
          <ArrowLeft className="w-4 h-4" />
          {user ? 'Quay lại lịch sử đơn hàng' : 'Quay lại trang chủ'}
        </Link>
      ...
      <div className="border border-gray-200 rounded-lg p-6 mb-6">
        ...
        {order.items.map((item, index) => {
          ...
          const fallbackImage = `https://source.unsplash.com/400x500/?${encodeURIComponent(item.productName || 'jewelry')}`;
          const imageSrc = resolveImageSrc(String(item.image || '').trim(), 'products') || fallbackImage;
                ...
                <div className="w-16 h-16 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
                  <img
                    ...
                    onError={(e) => {
                      ...
                      target.src = `https://source.unsplash.com/400x500/?${encodeURIComponent(item.productName)}`;
                    }}
                  />
                </div>
                <div>
                  ...
                  {showSizeStock && (
                    <p className={`text-sm ${isSizeOutOfStock ? 'text-red-600' : 'text-emerald-600'}`}>
                      {isSizeOutOfStock
                        ? 'Hiện tại: size đã hết hàng'
                        : `Hiện tại: còn ${currentSizeStock} sản phẩm cho size này`}
                    </p>
                  )}
                  {hasSale && (
      ...
      <div className="border-t border-gray-200 mt-4 pt-4 space-y-2 mb-6">
  ...
  return (
    <>
      {user ? (
        ...
        <div className="min-h-screen bg-white pt-24 pb-16">
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto">{orderDetailContent}</div>
          </div>
        </div>
      )}
